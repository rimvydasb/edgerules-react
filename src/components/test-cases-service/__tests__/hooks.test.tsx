...
function ResultProbe({service, testCaseId, path}: {service: TestCasesService; testCaseId: string; path: string}) {
    const result = useTestResult(service, testCaseId, path);
    return <span data-testid="value">{result ? String(result.value) : 'none'}</span>;
}
...
describe('useTestResult', () => {
    it('reflects a saved result and clears when the result set is cleared', async () => {
        ...
        render(<ResultProbe service={service} testCaseId={testCase.id} path="age" />);
