...
function PlainCellEditor({
  ...
  onCommit,
  onCancel,
}: PlainCellEditorProps): ReactElement {
      ...
      >
        ...
        {state?.withType ? (
          <Select
            ...
            size="small"
            onChange={(event) =>
              setType(event.target.value as PortableTypeReference)
            }
...
export function DecisionTableEditor({
  ...
  const [definition, setDefinition] =
    useState<PortableRulesetDefinition | null>(null);
  const [schema, setSchema] = useState<PortableRulesetSchema | undefined>(
    undefined,
  );
  ...
  const applyDefaultCell = useCallback(
    (name: string, text: string): void => {
      ...
      const cellPath =
        name === SCALAR_OUTPUT ? `${path}.default` : `${path}.default.${name}`;
  ...
  const renderRuleRow = (
    ...
    const whenCells: ReactNode =
      row.when.kind === 'expression' ? (
        <TableCell
          key="when-expression"
          colSpan={Math.max(model.inputs.length, 1)}
        >
          {editingKey ===
          cellKey({ kind: 'when-expression', row: rowIndex }) ? (
            ...
            <DisplayCell
              ...
              onStartEdit={() =>
                setEditing({ kind: 'when-expression', row: rowIndex })
              }
        ...
        {!readOnly && (
          <TableCell sx={{ width: 34, p: 0 }} align="center">
            <IconButton
              ...
              onClick={(event) =>
                setRowMenu({ anchor: event.currentTarget, row: rowIndex })
              }
            >
  ...
  const handleColumnMenuAction = (action: string): void => {
    ...
    if (
      ...
      setDialog({
        ...
        onSubmit: (name) =>
          applyDefinition(
            withOutputColumnRenamed(definition, column.name, name),
          ),
      });
    }
    if (action === 'delete') {
    ...
    switch (action) {
      case 'add-input':
        setDialog({
          ...
          withType: true,
          onSubmit: (name, type) =>
            applyDefinition(withInputColumnAdded(definition, name, type)),
        });
        break;
      case 'add-output':
        setDialog({
          ...
          onSubmit: (name) =>
            applyDefinition(withOutputColumnAdded(definition, name)),
        });
        break;
      case 'add-default': {
        const defaultNode = model.scorecard
          ...
          : (Object.fromEntries(
              model.outputs.map((column) => [
                column.name,
                column.typeLabel === 'number'
                  ? '0'
                  : column.typeLabel === 'boolean'
                    ? 'false'
                    : "''",
              ]),
            ) as PortableContext);
  ...
  return (
    <Box
      ...
      sx={{
        ...
        '& .tok-keyword': {
          color: (theme) =>
            theme.palette.mode === 'dark' ? '#c792ea' : '#7b1fa2',
        },
        '& .tok-atom': {
          color: (theme) =>
            theme.palette.mode === 'dark' ? '#d19a66' : '#e65100',
        },
        '& .tok-bool': {
          color: (theme) =>
            theme.palette.mode === 'dark' ? '#56b6c2' : '#0b7285',
        },
        '& .tok-number': {
          color: (theme) =>
        ...
        '& .tok-typeName': {
          color: (theme) =>
            theme.palette.mode === 'dark' ? '#4ec9b0' : '#267f99',
        },
        '& .tok-propertyName': {
          color: (theme) =>
            theme.palette.mode === 'dark' ? '#9cdcfe' : '#001080',
        },
        '& .tok-variableName': {
          color: (theme) =>
            theme.palette.mode === 'dark' ? '#9cdcfe' : '#0070c1',
        },
        '& .tok-function': {
          color: (theme) =>
            theme.palette.mode === 'dark' ? '#dcdcaa' : '#795e26',
        },
        '& .tok-self': {
          color: (theme) =>
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
        ...
        <Select
          ...
          onChange={(event) =>
            applyDefinition(
              withHitPolicy(definition, event.target.value as HitPolicy),
            )
          }
          renderValue={(value) => {
            const policy = HIT_POLICIES.find(
        ...
        {!readOnly && (
          <IconButton
            ...
            onClick={(event) => setTableMenu(event.currentTarget)}
          >
            <MoreVertIcon />
          </IconButton>
        )}
          ...
          <TableRow>
            <TableCell align="center" sx={{ width: 34 }}>
              <Tooltip
                title={
                  HIT_POLICIES.find(
                    (policy) => policy.value === model.hitPolicy,
                  )?.label ?? ''
                }
              >
                <span>
      ...
      >
        <MenuItem onClick={() => handleRowMenuAction('duplicate')}>
          Duplicate rule
        </MenuItem>
        <MenuItem onClick={() => handleRowMenuAction('move-up')}>
          Move up
        </MenuItem>
        <MenuItem onClick={() => handleRowMenuAction('move-down')}>
          Move down
        </MenuItem>
        {rowMenu !== null && model.rows[rowMenu.row]?.when.kind === 'cells' ? (
          ...
          <MenuItem onClick={() => handleRowMenuAction('to-cells')}>
            Use column conditions
          </MenuItem>
        )}
        <MenuItem
          onClick={() => handleRowMenuAction('delete')}
          sx={{ color: 'error.main' }}
        >
          Delete rule
        </MenuItem>
      ...
      >
        ...
        columnMenu.column.name !== SCALAR_OUTPUT ? (
          <MenuItem onClick={() => handleColumnMenuAction('rename')}>
            Rename column…
          </MenuItem>
        ) : null}
        <MenuItem
          onClick={() => handleColumnMenuAction('delete')}
          sx={{ color: 'error.main' }}
        >
          Delete column
        </MenuItem>
      ...
      >
        <MenuItem onClick={() => handleTableMenuAction('add-input')}>
          Add input column…
        </MenuItem>
        {!model.scorecard ? (
          <MenuItem onClick={() => handleTableMenuAction('add-output')}>
            Add output column…
          </MenuItem>
        ) : null}
        {model.hitPolicy !== 'collect-matches' && !model.defaultRow ? (
          <MenuItem onClick={() => handleTableMenuAction('add-default')}>
            Add default row
          </MenuItem>
        ) : null}
        {model.defaultRow ? (
          <MenuItem onClick={() => handleTableMenuAction('remove-default')}>
            Remove default row
          </MenuItem>
        ) : null}
      </Menu>
