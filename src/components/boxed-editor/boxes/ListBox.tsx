...
export function ListBox({ node, depth }: { node: ListRenderNode; depth: number }): ReactElement {
  const list = useListActions();
  const children = node.children ?? [];
  const renderItem = (child: BoxedRenderNode): ReactElement => {
    const item = { ...child, parentListLength: children.length, parentListTerminal: node.list.terminal } as BoxedRenderNode;
    return <ListItemBox key={child.id} node={item} depth={depth + 1} />;
  };
  return <BoxFrame node={node} depth={depth} header={<BoxHeader node={node} editable />} value={<Typography color="text.secondary">{node.list.loaded} list items</Typography>} type={<BoxTypeChip schema={node.schema} />}
    actions={<>{node.list.terminal && <IconButton size="small" aria-label={`Add item to ${node.path}`} onClick={() => list.addItem(node)}><AddIcon fontSize="small" /></IconButton>}<MetadataAction node={node} /><FieldActions node={node} /></>}
  >
    <CollectionChildren path={node.path} children={children} renderItem={renderItem} />
