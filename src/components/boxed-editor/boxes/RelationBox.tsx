...
export function RelationBox({
  ...
  const columns =
    children[0] && isObject(children[0].authored)
      ? Object.keys(children[0].authored).filter((key) => !key.startsWith('@'))
      : [];
  ...
  return (
    <BoxFrame
      ...
      value={
        <Typography color="text.secondary">
          {node.list.loaded} relation rows
        </Typography>
      }
    ...
    >
      {columns.length > 0 && (
        ...
        >
          {columns.map((column) => (
            <Typography key={column} role="columnheader" variant="caption">
              {column}
            </Typography>
          ))}
        </Box>
      ...
      >
        ...
        {node.list.terminal && (
          <RelationColumnActions node={node} columns={columns} />
        )}
      </Box>
    </BoxFrame>
  );
