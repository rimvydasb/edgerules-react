...
export function InvocationBox({
  node,
  depth,
}: {
  node: InvocationRenderNode;
  depth: number;
}): ReactElement {
  const invocation = useInvocationActions();
  const value = isObject(node.authored) ? (
    <StaticExpression value={invocationText(node.authored)} />
  ) : null;
  return (
    <BoxFrame
      node={node}
      ...
      actions={
        <>
          <Button
            size="small"
            aria-label={`Edit invocation ${node.path}`}
            onClick={() => invocation.edit(node)}
          >
            Invocation
          </Button>
    ...
    >
      {node.children?.map((child) => (
        <InvocationArgumentBox key={child.id} node={child} depth={depth + 1} />
      ))}
    </BoxFrame>
  );
}
