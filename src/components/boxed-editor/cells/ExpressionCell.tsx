...
import {CodeEditorCell, type CodeEditorEmbedContext} from '../../code-editor-cell';
...
import {authoredEntries, childPath, formatPortableValue, indexedPath, isRecord} from '../service/portable-utils';
...
function parametersText(parameters: unknown): string {
    ...
    return Object.entries(parameters)
        .map(([name, type]) => {
            ...
            if (isRecord(type) && typeof type.type === 'string') return `${name}: ${type.type}`;
            return `${name}: any`;
        })
        .join(', ');
}

function typeDefinitionText(node: Record<string, unknown>, path: string, activePath: string): string {
...
function contextText(node: Record<string, unknown>, path: string, activePath: string): string {
 ...
 */
function buildExpressionEmbedContext(root: PortableRootContext, activePath: string): CodeEditorEmbedContext {
