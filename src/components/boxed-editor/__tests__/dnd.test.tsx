...
describe('dropRules: isValidDrop matrix', () => {
  it('a context field may drop on any context or the model root, never a complexType', () => {
    expect(isValidDrop(source({ kind: 'field', containerKind: 'context' }), target({ containerKind: 'context' }))).toBe(true);
    expect(isValidDrop(source({ kind: 'field', containerKind: 'context' }), target({ containerKind: 'model' }))).toBe(true);
    expect(isValidDrop(source({ kind: 'field', containerKind: 'context' }), target({ containerKind: 'complexType' }))).toBe(false);
  });

  it('a complexType field may only drop on a complexType', () => {
    expect(isValidDrop(source({ kind: 'field', containerKind: 'complexType' }), target({ containerKind: 'complexType' }))).toBe(true);
    expect(isValidDrop(source({ kind: 'field', containerKind: 'complexType' }), target({ containerKind: 'context' }))).toBe(false);
    expect(isValidDrop(source({ kind: 'field', containerKind: 'complexType' }), target({ containerKind: 'model' }))).toBe(false);
  });
  ...
  it('an optimisation-variable/-constraint may only reorder within its own group', () => {
    const variable = source({ kind: 'optimisation-variable', containerPath: 'opt.variables', containerKind: 'optimisation-variable-group' });
    expect(isValidDrop(variable, target({ containerKind: 'optimisation-variable-group', containerPath: 'opt.variables' }))).toBe(true);
    expect(isValidDrop(variable, target({ containerKind: 'optimisation-variable-group', containerPath: 'other.variables' }))).toBe(false);
    expect(isValidDrop(variable, target({ containerKind: 'optimisation-constraint-group', containerPath: 'opt.variables' }))).toBe(false);

    const constraint = source({ kind: 'optimisation-constraint', containerPath: 'opt.constraints', containerKind: 'optimisation-constraint-group' });
    expect(isValidDrop(constraint, target({ containerKind: 'optimisation-constraint-group', containerPath: 'opt.constraints' }))).toBe(true);
    expect(isValidDrop(constraint, target({ containerKind: 'optimisation-constraint-group', containerPath: 'other.constraints' }))).toBe(false);
  });
