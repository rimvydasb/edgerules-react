...
export function BoxHeader({ node, label = node.name ?? (node.path === '*' ? 'Model' : node.path), editable = false }: { node: BoxedRenderNode; label?: string; editable?: boolean }): ReactElement {
  ...
  return <>
    {editable && fields.editingPath === node.path
      ? <InputBase autoFocus value={fields.nameDraft} inputProps={{ 'aria-label': `Name ${node.path}` }} onChange={event => fields.setNameDraft(event.target.value)} onBlur={() => fields.commitRename(node)} onKeyDown={event => { if (event.key === 'Enter') fields.commitRename(node); if (event.key === 'Escape') fields.setNameDraft(node.name ?? ''); }} />
      : label}
