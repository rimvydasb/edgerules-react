...
describe('CodeEditor', () => {
    ...
    it('highlights EdgeRules syntax with stable token classes', async () => {
        const onChange = vi.fn();
        const {container} = render(<CodeEditor value={VALID_MODEL_DSL} onChange={onChange} service={service} />);
    ...
    it('surfaces real engine diagnostics as lint markers for invalid code', async () => {
        const onChange = vi.fn();
        const {container} = render(<CodeEditor value={INVALID_MODEL_DSL} onChange={onChange} service={service} />);
    ...
    it('navigates to a definition with F12', async () => {
        ...
        const view = getView(container);
        const usage = VALID_MODEL_DSL.indexOf('riskScore(applicant') + 'riskScore('.length + 'app'.length;
    ...
    it('does not allow edits when readOnly', async () => {
        const user = userEvent.setup();
        const onChange = vi.fn();
        const {container} = render(<CodeEditor value="{ a: 1 }" onChange={onChange} service={service} readOnly />);
