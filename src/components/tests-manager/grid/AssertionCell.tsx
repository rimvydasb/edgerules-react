...
export function AssertionCell({testCaseId, row}: {testCaseId: string; row: TestRow}): ReactElement {
    ...
    return (
        <Tooltip title={mismatch ? formatValue(cell.result?.value) : ''}>
            <TextField
                ...
                sx={[
                    FILL_CELL_SX,
                    mismatch ? {'& .MuiInputBase-input': {color: 'error.main', fontWeight: 600}} : false,
                ]}
            />
        </Tooltip>
    );
