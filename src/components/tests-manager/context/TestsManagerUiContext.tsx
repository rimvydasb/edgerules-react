...
const TestsManagerUiContext = createContext<
  TestsManagerUiContextValue | undefined
>(undefined);
...
export function TestsManagerUiProvider({
  children,
}: {
  children: ReactNode;
}): ReactElement {
  ...
  const value = useMemo(
    () => ({ pageIndex, setPageIndex, activeCell, setActiveCell }),
    [pageIndex, activeCell],
  );
  return (
    <TestsManagerUiContext.Provider value={value}>
      {children}
    </TestsManagerUiContext.Provider>
  );
}
...
export function useTestsManagerUiContext(): TestsManagerUiContextValue {
  ...
  if (!context) {
    throw new Error(
      'useTestsManagerUiContext must be used within a TestsManagerUiProvider',
    );
  }
  return context;
}
