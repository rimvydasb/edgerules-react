...
const TestsManagerContext = createContext<TestsManagerContextValue | undefined>(undefined);
...
export function TestsManagerProvider({
    ...
    return <TestsManagerContext.Provider value={value}>{children}</TestsManagerContext.Provider>;
}

export function useTestsManagerContext(): TestsManagerContextValue {
    const context = useContext(TestsManagerContext);
    if (!context) {
        throw new Error('useTestsManagerContext must be used within a TestsManagerProvider');
    }
    return context;
}
