...
async function pathEditorView(editorTestId: string): Promise<EditorView> {
  ...
  const view = EditorView.findFromDOM(
    editor.querySelector('.cm-editor') as HTMLElement,
  );
...
describe('TestsManager — pre-generated indexed paths', () => {
  it('pre-generates the zero-indexed element of every array, at every depth', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId('row-application.applicant'),
      ).toBeInTheDocument(),
    );
    expect(
      screen.getByTestId('row-application.applicant[0].name'),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId('row-application.applicant[0].creditLine[0].balance'),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId('row-application.applicant[0].creditLine[0].limit'),
    ).toBeInTheDocument();
  });
  ...
  it('duplicates an indexed row into the next free element, right below its source', async () => {
    ...
    await user.click(
      screen.getByLabelText('row menu application.applicant[0].name'),
    );
    ...
    expect(
      screen
        .getByTestId('row-application.applicant[0].name')
        .compareDocumentPosition(copy),
    ).toBe(Node.DOCUMENT_POSITION_FOLLOWING);
    expect(
      screen.getByLabelText('input application.applicant[1].name'),
    ).toBeInTheDocument();
  });
  ...
  it('deletes a duplicated row outright, leaving the derived one alone', async () => {
    ...
    await user.click(
      screen.getByLabelText('row menu application.applicant[1].name'),
    );
    await user.click(screen.getByRole('menuitem', { name: 'Delete row' }));

    await waitFor(() =>
      expect(
        screen.queryByTestId('row-application.applicant[1].name'),
      ).toBeNull(),
    );
    expect(
      screen.getByTestId('row-application.applicant[0].name'),
...
describe('TestsManager — editing a path', () => {
  it('opens an editor on click and repoints the row on commit, carrying its input value', async () => {
    ...
    await user.type(
      screen.getByLabelText('input application.applicant[0].name'),
      'Ann',
    );
    await user.click(document.body);

    await user.click(
      screen.getByLabelText('path application.applicant[0].name'),
    );
    const content = await retypePath(
      'path-editor-application.applicant[0].name',
      'application.applicant[3].name',
    );
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId('row-application.applicant[3].name'),
      ).toBeInTheDocument(),
    );
    expect(
      (
        screen.getByLabelText(
          'input application.applicant[3].name',
        ) as HTMLInputElement
      ).value,
    ).toBe('Ann');
    expect(
      screen.queryByTestId('row-application.applicant[0].name'),
    ).toBeNull();
  });
  ...
  it('marks a path the model does not declare while it is being typed', async () => {
    ...
    await waitFor(() =>
      expect(getComputedStyle(editor).borderColor).not.toBe(''),
    );
  ...
  it('lints nothing for a path the model declares — the first segment is not an unknown reference', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId('row-application.applicant[0].creditLine'),
      ).toBeInTheDocument(),
    );

    await user.click(
      screen.getByLabelText('path application.applicant[0].creditLine'),
    );
    const editor = await screen.findByTestId(
      'path-editor-application.applicant[0].creditLine',
    );

    // Let a lint cycle run, then assert nothing was marked anywhere in the cell.
    await new Promise((resolve) => setTimeout(resolve, 100));
    expect(
      editor.querySelector('.cm-lintRange-error, .cm-lintPoint-error'),
    ).toBeNull();
  });
  ...
  it('marks only the undeclared segment of a wrong path', async () => {
    ...
    expect(editor.querySelector('.cm-lintRange-error')?.textContent).toBe(
      'nope',
    );
  });
  ...
  it('completes addressable paths — not model built-ins — and hosts the popup outside the grid', async () => {
    ...
    const view = await pathEditorView(
      'path-editor-application.applicant[0].name',
    );
    view.focus();
    ...
    await waitFor(() => {
      const options = currentCompletions(view.state).map(
  ...
  it('shows a path the model no longer declares in the error colour', async () => {
    ...
    await waitFor(() =>
      expect(
        getComputedStyle(
          screen.getByLabelText('path application.applicant[1].name'),
        ).color,
      ).toContain('211, 47, 47'),
    );
  });
