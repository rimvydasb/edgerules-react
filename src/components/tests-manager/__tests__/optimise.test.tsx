...
describe('TestsManager — optimise subject', () => {
    it('runs end to end through a registered solver', async () => {
        ...
        render(<TestsManager service={service} modelName={uniqueModelName()} subjectId="factoryProduction" />);

        await waitFor(() => expect(screen.getByLabelText('input workers')).toBeInTheDocument());
        ...
        await waitFor(() => expect(screen.getByTestId('validation-status')).toHaveTextContent('optimal'));
        ...
        expect(screen.getByTestId('validation-solver')).toHaveTextContent('stub 1.0');
    });
    ...
    it('shows the missing-solver pre-flight banner when no solver is registered', async () => {
        ...
        await waitFor(() => expect(screen.getByTestId('missing-solver-banner')).toBeInTheDocument());
    });
});
