...
import {groupContextChildren, listTypeEntries, ROOT_FETCH_PATH, ROOT_PATH, type NamedEntry} from './tree-model';
import {KindIcon, type IconKind} from './icons';

type GetFilter = 'FIELDS' | 'TYPE_DEFINITIONS' | 'FUNCTION_DEFINITIONS' | 'EXTERNAL_DEFINITIONS' | 'ALL';
...
export function ProjectExplorer({
    ...
    const [rootTypes] = useState<PortableNode | PortableError>(() => service.get(ROOT_FETCH_PATH, 'TYPE_DEFINITIONS'));
    ...
    function handleExpandedItemsChange(_event: SyntheticEvent | null, itemIds: string[]): void {
        ...
        const toFetch = newlyExpanded.filter((id) => isFetchableContextPath(id) && !contextCache.has(id));
        ...
        return (
            <TreeItem key={itemId} itemId={itemId} label={renderLabel('types', 'Types')}>
                {entries.map((entry) => {
                    ...
                    return <TreeItem key={entryItemId} itemId={entryItemId} label={renderLabel('type', entry.name)} />;
                })}
            </TreeItem>
        );
    }
    ...
    function renderOrderedEntry(entry: NamedEntry): ReactNode {
        if (entry.kind === 'func') {
            ...
            return <TreeItem key={entry.path} itemId={entry.path} label={renderLabel('func', `${entry.name}()`)} />;
        }

        if (entry.kind === 'dt') {
            actions.set(entry.path, () => onOpenDecisionTable?.(entry.path));
            return <TreeItem key={entry.path} itemId={entry.path} label={renderLabel('dt', `${entry.name}()`)} />;
        }
