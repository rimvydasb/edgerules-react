...
import type {ListItemDraft, RelationColumnDraft, SignatureDraft} from '../boxed-editor-types';
    ...
    return (
        ...
        >
                        ...
                        onClick={() =>
                            setDraft((current) =>
                                current
                                    ? {
                                          ...current,
                                          parameters: current.parameters.filter((_, itemIndex) => itemIndex !== index),
                                      }
                                    : current,
                            )
                        }
                ...
                onClick={() =>
                    setDraft((current) =>
                        current
                            ? {
                                  ...current,
                                  parameters: [...current.parameters, {name: '', type: 'number'}],
                              }
                            : current,
                    )
                }
            ...
            <TextField
                ...
                onChange={(event) =>
                    setDraft((current) => (current ? {...current, returnType: event.target.value} : current))
                }
            />
        </EditorDialog>
    );
...
export function RelationColumnForm({
    ...
    const title = draft?.action === 'add' ? 'Add relation column' : 'Delete relation column';
    return (
        <EditorDialog
            open={Boolean(draft)}
            title={title}
