...
import {FunctionSignatureForm, ListItemForm, RelationColumnForm} from './forms/EditorForms';
import {
...
import type {BoxedEditorProps, ListItemDraft, RelationColumnDraft, SignatureDraft} from './boxed-editor-types';
import {
    LIST_PAGE_SIZE,
    childPath,
    findNode,
...
const siblingCollision: CollisionDetection = (args) => {
    const activeGroup = (args.active.data.current?.reorder as BoxedSortableMetadata | undefined)?.groupId;
    if (!activeGroup) return [];
    return closestCenter({
        ...args,
        droppableContainers: args.droppableContainers.filter(
            (container) =>
                (container.data.current?.reorder as BoxedSortableMetadata | undefined)?.groupId === activeGroup,
        ),
    });
};

function reorderContextFields(authored: Record<string, unknown>, names: string[]): PortableNode {
    const metadata = Object.entries(authored).filter(([name]) => name.startsWith('@'));
    const fields = new Map(Object.entries(authored).filter(([name]) => !name.startsWith('@')));
    ...
    return Object.fromEntries([...metadata, ...reordered, ...remaining]) as PortableNode;
}
...
export function BoxedEditor({
    ...
    const load = useCallback(
        (nextSnapshot: PortableRootContext, resetExpansion: boolean): boolean => {
            ...
            if (resetExpansion)
                setExpanded(
                    new Set([next.id, ...(path !== '*' ? (next.children?.map((child) => child.id) ?? []) : [])]),
                );
            return true;
        },
        [path, service],
    ...
    const commitExpression = useCallback(
        (node: BoxedRenderNode, text: string): void => {
            ...
            if (node.functionBody) {
                const definition = snapshot && resolveAuthoredPath(snapshot, node.functionBody.path);
    ...
    const addRelationshipRow = useCallback(
        (node: RelationRenderNode): void => {
            ...
            if (!draft) {
                showError(node.path, 'The first relationship row is required to establish column types.');
                return;
            }
    ...
    const duplicateListItem = useCallback(
        (node: BoxedRenderNode): void => {
            ...
            const result = service.set(
                ...
                list?.kind === 'relation' ? clearRelationshipContextMetadata(node.authored) : node.authored,
            );
            if (isPortableError(result)) {
                showError(node.path, result);
                return;
    ...
    const reorderRootFields = useCallback(
        (names: string[], values: ReadonlyMap<string, PortableNode>): PortableError | undefined => {
    ...
    const reorderSiblings = useCallback(
        (activeId: string, overId: string): void => {
            ...
            if (!active || !over || !activeSortable || !overSortable || activeSortable.groupId !== overSortable.groupId)
                return;
            ...
            if (activeSortable.ownerKind === 'collection') {
                result = service.set(
                    ...
                    reordered.map((child) =>
                        owner.kind === 'relation' ? clearRelationshipContextMetadata(child.authored) : child.authored,
                ...
                const originalNames = siblings.flatMap((child) => (child.name ? [child.name] : []));
                const values = new Map(
                    siblings.flatMap((child) => (child.name ? [[child.name, child.authored] as const] : [])),
                );
                ...
                if (isPortableError(result)) {
                    ...
                    if (isPortableError(rollback)) {
                        setFatalError(`Could not restore root field order: ${rollback.message}`);
                        return;
                    }
                }
            } else {
    ...
    const reorderRelationColumns = useCallback(
        (ownerPath: string, activeId: string, overId: string): void => {
            ...
            const from = relation.columns.findIndex((column) => column.id === activeId);
            const to = relation.columns.findIndex((column) => column.id === overId);
            if (from < 0 || to < 0 || from === to) return;
            const columns = arrayMove(relation.columns, from, to).map((column) => column.name);
            const items = (relation.children ?? []).map((child) =>
                isObject(child.authored)
                    ? clearRelationshipContextMetadata(reorderContextFields(child.authored, columns))
                    : child.authored,
            );
    ...
    const finishDrag = useCallback(
        ({active, over}: DragEndEvent): void => {
            if (!over) return;
            const sortable = active.data.current?.reorder as BoxedSortableMetadata | undefined;
            if (sortable?.ownerKind === 'relation-column') {
                reorderRelationColumns(sortable.ownerPath, String(active.id), String(over.id));
                return;
            }
        ...
        (node: RelationRenderNode, source: string, name: string): void => {
            ...
            const items = (node.children ?? []).map((child) => {
                ...
                return clearRelationshipContextMetadata(
                    Object.fromEntries(
                        Object.entries(child.authored).map(([key, value]) => [key === source ? nextName : key, value]),
                    ) as PortableNode,
                );
            });
    ...
    const openListItem = (node: BoxedRenderNode): void => setListItemDraft({path: node.path, value: '0'});
    return (
        <BoxedEditorProvider
            state={{
                readOnly,
            ...
            list={{
                addItem: (node) => (node.kind === 'relation' ? addRelationshipRow(node) : openListItem(node)),
        ...
        >
            <DndContext sensors={sensors} collisionDetection={siblingCollision} onDragEnd={finishDrag}>
                <Box
                    className={className}
                    sx={{
                        border: '1px solid',
                ...
                >
                    {path === '*' && (snapshot['@model-name'] || snapshot['@model-version']) && (
                        <Box
                            sx={{
                                px: 1,
                                py: 0.5,
                        ...
                        >
                            <Typography variant="subtitle2">
                                ...
                                {snapshot['@model-version'] ? ` · ${snapshot['@model-version']}` : ''}
                            </Typography>
                        </Box>
                    )}
