...
export function EditorLinkBox({
  node,
  depth,
}: {
  node: EditorLinkRenderNode;
  depth: number;
}): ReactElement {
  const navigation = useEditorNavigation();
  const kind = (
    isObject(node.authored) ? node.authored['@kind'] : undefined
  ) as BoxedEditorTargetKind;
  const text =
    kind === 'type-definition'
      ? 'Open Types Editor'
      : kind === 'ruleset'
        ? 'Open Decision Table Editor'
        : 'Open Loop Editor';
  const value = navigation.open ? (
    <Button
      size="small"
      onClick={() => navigation.open?.({ path: node.path, kind })}
    >
      {text}
    </Button>
  ) : (
