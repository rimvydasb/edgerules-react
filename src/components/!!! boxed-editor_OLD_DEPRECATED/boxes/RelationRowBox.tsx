...
import {useBoxedEditorState, useBoxedNodeRenderer, useExpressionActions} from '../BoxedEditorProvider';
            ...
            >
                {node.sortable && (
                    <IconButton
                        ...
                        sx={{
                            cursor: state.readOnly ? 'default' : sortable.isDragging ? 'grabbing' : 'grab',
                            touchAction: 'none',
                        }}
                        {...sortable.attributes}
                        {...sortable.listeners}
            ...
            {columns.map((column) => {
                ...
                return (
                    <TableCell
                        ...
                        title={drillable ? cell?.name : cell ? cellCode(cell.authored) : undefined}
                        tabIndex={(drillable || (!state.readOnly && editable)) && !editing ? 0 : undefined}
                        ...
                        sx={{
                            ...
                            cursor: drillable ? 'pointer' : !state.readOnly && editable ? 'cell' : 'default',
                            overflow: 'hidden',
                            borderRight: '1px solid',
                            borderColor: 'divider',
                            '& code': {
                        ...
                        {editing ? (
                            ...
                            <Fragment>
                                <Box sx={{display: 'flex', alignItems: 'center', minWidth: 0}}>
                                    {drillable &&
                                        (expanded ? (
                                            <ExpandLessIcon fontSize="small" sx={{mr: 0.5, flex: '0 0 auto'}} />
                                        ) : (
                                            <ExpandMoreIcon fontSize="small" sx={{mr: 0.5, flex: '0 0 auto'}} />
                                        ))}
                                    {drillable ? (
                                        <Box sx={{minWidth: 0, fontWeight: 600}}>{cell.name ?? column.name}</Box>
                                    ) : (
                                        <Box sx={{minWidth: 0, overflow: 'hidden'}}>{cellCode(cell.authored)}</Box>
                                    )}
                                </Box>
                                {drillable && expanded && (
                                    <Box
