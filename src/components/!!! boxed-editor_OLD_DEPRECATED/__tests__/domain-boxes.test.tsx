...
describe('boxed-editor domain boxes', () => {
    ...
    it('ExpressionBox renders a static expression, reports its error, and activates only expression editing', async () => {
        ...
        expect(screen.queryByRole('button', {name: 'Rename answer'})).not.toBeInTheDocument();
        await userEvent.click(screen.getByRole('button', {name: 'Edit name answer'}));
        expect(commands.field.startRename).toHaveBeenCalledWith(value);
        await userEvent.click(within(screen.getByRole('row', {name: 'answer'})).getAllByRole('cell')[3]);
    ...
    it('RelationBox composes RelationRowBox and owns relation column commands', async () => {
        ...
        expect(screen.getByRole('row', {name: 'people[0]'})).toHaveTextContent('Ada');
