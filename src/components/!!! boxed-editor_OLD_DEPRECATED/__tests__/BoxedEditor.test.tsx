...
describe('BoxedEditor', () => {
    ...
    it('renders leading drag handles for authored sibling fields only in editable mode', () => {
        ...
        expect(screen.getByRole('button', {name: 'Drag application'})).toBeInTheDocument();
        expect(screen.getByRole('button', {name: 'Drag monthly'})).toBeInTheDocument();
    ...
    it('keeps drag handles visible but disables all editing in read-only mode', async () => {
        const user = userEvent.setup();
        const {container} = render(<BoxedEditor service={service()} path="*" readOnly />);
        const application = screen.getByRole('row', {name: 'application'});
        const payment = screen.getByRole('row', {name: 'payment'});

        expect(within(application).getByRole('button', {name: 'Drag application'})).toBeDisabled();
        expect(within(payment).getByRole('button', {name: 'Drag payment'})).toBeDisabled();
        expect(screen.queryByRole('button', {name: 'Edit name payment'})).not.toBeInTheDocument();

        await user.click(within(payment).getAllByRole('cell')[3]);
        expect(container.querySelector('.cm-editor')).not.toBeInTheDocument();
        expect(screen.queryByRole('textbox', {name: 'Name payment'})).not.toBeInTheDocument();
    });
    ...
    it('commits an expression through one active CodeEditorCell and emits the refreshed snapshot', async () => {
        ...
        await user.click(within(screen.getByRole('row', {name: 'answer'})).getAllByRole('cell')[3]);
    ...
    it('edits a scalar function body through its owning function definition', async () => {
        ...
        await user.click(within(screen.getByRole('row', {name: 'monthly.result'})).getAllByRole('cell')[3]);
        const editor = container.querySelector<HTMLElement>('.cm-content');
        ...
        expect(onChange).toHaveBeenCalledTimes(1);
        expect(container.querySelectorAll('.cm-editor')).toHaveLength(0);
        expect(screen.getByRole('row', {name: 'monthly.result'})).toHaveTextContent('amount / 6');
    });
    ...
    it('writes complete typed inputs and supports context add, rename, duplicate, and delete', async () => {
        ...
        expect(instance.toPortable().application as object).toHaveProperty('field', 0);
        expect(Object.keys(instance.toPortable().application as object).at(-1)).toBe('field');
    ...
    it('changes a cell from number to input to string through one inline editor', async () => {
        ...
        expect(screen.getByRole('row', {name: 'mutable'})).toHaveTextContent('number · input');

        await edit('<number, default: 5>', '"text"');
        expect(instance.toPortable().mutable).toBe("'text'");
        expect(screen.getByRole('row', {name: 'mutable'})).toHaveTextContent('string · computed');
    });
    ...
    it('rolls back a rename when linked validation exposes an unresolved reference', async () => {
        ...
        await user.click(screen.getByRole('button', {name: 'Expand application'}));
        await user.click(screen.getByRole('button', {name: 'Edit name application.amount'}));
    ...
    it('edits an invocation as one DSL cell without exposing metadata editing', async () => {
        ...
        expect(screen.queryByRole('button', {name: 'Expand payment'})).not.toBeInTheDocument();
        await user.click(within(invocationRow).getAllByRole('cell')[3]);
        const editor = container.querySelector<HTMLElement>('.cm-content');
        expect(editor).not.toBeNull();
        expect(editor).toHaveTextContent('monthly(application.amount)');
        await user.click(editor!);
        await user.keyboard('{Control>}a{/Control}monthly(application.amount * 2){Enter}');
    ...
    it('pages and edits CRUD-addressable scalar lists without expanding computed arrays', async () => {
        ...
        expect(screen.queryByRole('button', {name: 'Drag nums[0]'})).not.toBeInTheDocument();
        ...
        expect(screen.getByRole('button', {name: 'Drag nums[0]'})).toBeInTheDocument();
    ...
    it('renders literal record lists as relations and writes row and column changes atomically', async () => {
      ...
      people: [{ name: "A"; age: 1 }, { name: "B"; age: 2 }]
        ...
        expect(screen.getByRole('row', {name: 'people[0]'})).toHaveTextContent("'A'1");
        ...
        expect((instance.toPortable().people as {expression: string}).expression).toContain('name: ""; age: 0');
    ...
    it('drills from a relationship cell through nested contexts to scalar fields', async () => {
      ...
      people: [{
        ...
        expect(screen.queryByText('1 relationship rows · 2 columns')).not.toBeInTheDocument();
        ...
        contact.focus();
        await user.keyboard('{Enter}');
        expect(screen.queryByRole('row', {name: 'people[0].contact.address'})).not.toBeInTheDocument();

        await user.click(screen.getByRole('button', {name: 'Duplicate people[0]'}));
