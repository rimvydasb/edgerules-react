...
export function CollectionChildren({
    ...
    renderItem: (child: BoxedRenderNode) => ReactNode;
}): ReactElement {
    const [scrollTop, setScrollTop] = useState(0);
    if (children.length <= 100) return <SortableChildren nodes={children}>{children.map(renderItem)}</SortableChildren>;
    const rowHeight = 42;
    ...
    return (
        <SortableChildren nodes={children}>
            <Box
                aria-label={`Virtualized rows ${path}`}
