...
export function BoxedEntityNode({
    node,
    depth,
    actions,
    ...
    switch (node.kind) {
        case 'context':
            return (
                <ContextBox node={node} depth={depth} actions={actions} suppressFieldActions={suppressFieldActions} />
            );
        case 'expression':
            return (
                <ExpressionBox
        ...
        case 'input':
            return <InputBox node={node} depth={depth} actions={actions} suppressFieldActions={suppressFieldActions} />;
        case 'list':
            return <ListBox node={node} depth={depth} />;
        case 'relation':
            return <RelationBox node={node} depth={depth} />;
        case 'function':
