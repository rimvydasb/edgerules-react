...
- [x] Add `ExecutingHarness` to `BoxedEditor.stories.tsx` — a wrapper rendering `BoxedEditor` plus two read-only
      captions.
- [x] `data-testid="live-result"` — the output of `mutable.execute(method, input)`, re-run on every `onChange`,
      keyed by a revision counter (mirror `TestCasesAndTestRunnerHarness`'s existing `revision` pattern).
- [x] **Render failures too.** A structural edit that breaks linking must show as a failed run, not a stale previous
...
- [x] `data-testid="live-model"` — `JSON.stringify(service.toPortable())`, so structural edits with no execution
      effect are still assertable.
- [x] `data-testid="boxed-change-count"` — keep the existing counter; it is the only way to assert "`onChange` fired
      exactly once per **successful** commit, never on a rejected one".
- [x] Note in a code comment that `execute()` is **async** (`Promise<unknown>`), so every test asserting on
...
- [x] Add `highs` as a **devDependency** and wire a real solver handler in the story:
...
- [x] Verify it runs in the browser (highs-js is Emscripten-based; confirm the Storybook build serves its wasm asset).
- [x] Only if the real solver cannot be made to work in the browser: fall back to a canned-solution stub, label it
...
- [x] `BlankModel` — `fromCode('{}')` (or the minimal equivalent), wrapped in `ExecutingHarness`. The **only** fixture
      [Phase 11](improvement-phase-11.md) may load.
- [x] `FunctionCrudPlayground` — one existing multi-arg function (so tests can mutate an existing signature) and room
      to grow a new one from zero, with fixed inputs so execution correctness is visible after every structural edit.
- [x] `DecisionTableCrudPlayground` — a ruleset with **typed** condition columns (`number`/`string`/`date` —
      ...
      is open.
- [x] `RelationCrudPlayground` — a relation with 3+ **mixed-type** columns (string, number, and a complex/drill-down
...
- [x] Wire `documentationService` into at least one fixture so description-overlay tests
...
- [x] Execute every new fixture once in the Storybook UI before writing any test against it. A fixture that does not
...
- [x] `expectLiveResult(page, expected)` — auto-retrying assertion on `live-result`.
- [x] `expectLiveModel(page, matcher)` — auto-retrying assertion on `live-model`.
- [x] `expectRowError(page, path, text)` — asserts `row-error-${path}` (the channel
...
- [x] `expectNoRowError(page, path)` — the negative form, for "this should have succeeded" assertions.
- [x] `columnHeaders(page, rowPath)` — reads the ordered column names of a `function`/`ruleset`/`relation` header.
      Text-based for now; switch to the `column-${rowPath}-${columnName}` testid when
      [Phase 4](improvement-phase-4.md) adds it.
- [x] `dragRow(page, fromPath, toPath)` — a reusable `@dnd-kit`-compatible drag gesture (mouse down, move in steps,
      up). `@dnd-kit` ignores instantaneous moves, so this needs intermediate steps; get it right once here.
- [x] Create `e2e/boxed-editor/columnHelpers.ts` as an empty, documented module — [Phase 9](improvement-phase-9.md)
...
- [x] `test('executes the model and shows a live result')` — open `BlankModel`, add one field through the UI, and
...
- [x] `test('shows an execution failure instead of a stale result')` — commit something that breaks linking and assert
      `live-result` shows the error, not the previous value.
- [x] `test('solves an optimisation end to end')` — open `OptimisationCrud` through `ExecutingHarness` and assert a
...
- [x] `ExecutingHarness` renders `live-result`, `live-model` and `boxed-change-count`, and shows execution errors.
- [x] All four new fixtures exist, link, and execute.
- [x] `highs` is wired (or the fallback is documented here with its restriction).
- [x] Every helper in 1.4 exists and is exported.
- [x] The three smoke tests in 1.5 pass.
- [x] `npx playwright test e2e/boxed-editor` is green and `tsc --noEmit` is clean.
