...
- `../edgerules-v2/doc/architecture/` — design specs (`ARCHITECTURE.md` first; `API_SPEC.md` and
  `CRUD_SPEC.md` for the `execute`/`get`/`set`/`remove`/`rename` API and the linking/`tast_dirty` contract;
  `dsl/`+`EBNF.md` for grammar). `*_SPEC.md` = authoritative; `*_STORY.md` = implementation history, may be stale —
  verify against the installed package version.
...
detected bugs, append `docs/engine-bug-reports.md` with a new entry describing the bug.
